...
OPTIONS:
 ...
 ARGS per Action:
    ...
    init-helm-repository       : initialize a local helm chart repository
      ...
      --chartmuseumImage       : chartmuseum image (default: ghcr.io/helm/chartmuseum)
    ...
    delete-helm-repository     : delete a local helm chart repository
"
...
init_helm_repository() {
    ...
    "${scriptDir}"/airgap.sh helm repository init\
        ${helm_user:+'--username' "$helm_user"} \
        ${helm_pass:+'--password' "$helm_pass"} \
        ${chart_dir:+'--dir' "$chart_dir"} \
        ${chartmuseum:+'--chartmuseum' "$chartmuseum"} \
        ${chartmuseumImage:+'--chartmuseumImage' "$chartmuseumImage"} \
        ${chartmuseumImageTag:+'--chartmuseumImageTag' "$chartmuseumImageTag"}
}
...
delete_helm_repository() {
    echo "-------------Delete helm repository-------------"
    "${scriptDir}"/airgap.sh helm repository delete
}
