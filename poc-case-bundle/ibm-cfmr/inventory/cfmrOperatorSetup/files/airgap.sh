...
CHART_DIR=/tmp/cases/charts

CHARTMUSEUM_NAME=

CHARTMUSEUM_IMAGE=ghcr.io/helm/chartmuseum

CHARTMUSEUM_IMAGE_TAG=v0.13.1
...
HELM_PASSWORD_GENERATED=false

PRIVATE_HELM_REPO="private-helm-repo"
...
print_usage() {
    ...
    echo "   helm              Configure local helm chart repository"
...
#
# Parses argument for 'helm' action
#
parse_helm_arguments() {
    if [[ "$#" == 0 ]]; then
        print_helm_usage
    ...
    while [ "$1" != "" ]; do
        ...
        repository)
            shift
            parse_helm_repository_arguments "$@"
            break
            ;;
        -h | --help)
            print_helm_usage
...
# Prints usage menu for 'helm' action
#
print_helm_usage() {
    script_name=`basename ${0}`
    echo "Usage: ${script_name} helm [service|secret] [OPTIONS]..."
    echo ""
    echo "Run a private helm chart repository"
    echo "for helm chart push/pull functions."
    echo ""
    echo "Options:"
    echo "  repository"
    echo "    init               Initialize a helm chart repository"
    echo "    delete             Delete a helm chart repository"
    echo "  -h, --help           Print usage information"
    echo ""
}
...
parse_helm_repository_arguments() {
    ...
    while [[ "$1" != "" ]]; do
        ...
        init)
            shift
            do_helm_repository_init "$@"
            break
            ;;
        load)
            shift
            do_helm_repository_load "$@"
            break
            ;;
        delete)
            shift
            do_helm_repository_delete "$@"
            break
            ;;
        -h | --help)
            print_helm_repository_usage
...
#
# Prints usage menu for 'helm repository' action
#
print_helm_repository_usage() {
    script_name=`basename ${0}`
    echo "Usage: ${script_name} helm repository [init|load|delete]"
    echo ""
    echo "A helper to intialize, load and delete helm repository"
    echo ""
    echo "Options:"
    echo "   init         Initialize a helm repository"
    echo "   load         Load a helm repository"
    echo "   delete       Delete a helm chart repository"
    echo "   -h, --help   Print usage information"
    echo ""
}
...
# ---------- Helm Repsitory functions ----------

#
# Initializes helm repository
#
do_helm_repository_init() {
    # parses arguments
    parse_helm_repository_init_arguments "$@"

    # validates arguments
    validate_helm_repository_init_arguments

    # validates required tools
    validate_helm_repository_init_required_tools

    # apply oc command
    echo "[INFO] Creating private-helm-repo namespace"
    oc_cmd="oc new-project ${PRIVATE_HELM_REPO}"
    echo "${oc_cmd}"
    eval ${oc_cmd}
    ...
    # apply oc command
    echo "[INFO] Adding SCC to private-helm-repo"
    oc_cmd="oc adm policy add-scc-to-user privileged system:serviceaccount:${PRIVATE_HELM_REPO}:${PRIVATE_HELM_REPO}"
    echo "${oc_cmd}"
    eval ${oc_cmd}
    ...
    # apply oc command
    echo "[INFO] Installing private-helm-repo"
    helm_cmd="helm install ${PRIVATE_HELM_REPO} -n "${PRIVATE_HELM_REPO}" --set env.existingSecretMappings.BASIC_AUTH_USER=${HELM_USERNAME} --set env.existingSecretMappings.BASIC_AUTH_PASS=${HELM_PASSWORD} --set image.repository=${CHARTMUSEUM_IMAGE} --set image.tag=${CHARTMUSEUM_IMAGE_TAG} --set env.open.AUTH_ANONYMOUS_GET=true --set serviceAccount.create=true --set serviceAccount.name="${PRIVATE_HELM_REPO}" --set env.open.DISABLE_API=false ${CHART_DIR}/${CHARTMUSEUM_NAME} --wait"
    echo "${helm_cmd}"
    eval ${helm_cmd}
    ...
    # apply oc command
    echo "[INFO] Expose private-helm-repo to be accessible"
    oc_cmd="oc expose svc ${PRIVATE_HELM_REPO}-chartmuseum"
    echo "${oc_cmd}"
    eval ${oc_cmd}
    ...
    # apply oc command
    oc_cmd="oc get routes -n ${PRIVATE_HELM_REPO} --no-headers"
    host=$(oc get routes -n ${PRIVATE_HELM_REPO} --no-headers | awk '{print $2}')
    termination=$(oc get routes -n ${PRIVATE_HELM_REPO} --no-headers | awk '{print $4}')
    echo "[INFO] Route URL for private-helm-repo to be used during loading of helm charts:"
    echo "$termination://$host"

    if [[ "$?" -ne 0 ]]; then
        exit 11
    fi

    if [[ "${HELM_PASSWORD_GENERATED}" == "true" ]]; then
        echo "[INFO] username = ${HELM_USERNAME}"
        echo "[INFO] password = ${HELM_PASSWORD}"
    fi
}
...
parse_helm_repository_init_arguments() {
    ...
    while [[ "$1" != "" ]]; do
        ...
        -cm | --chartmuseum)
            ...
            CHARTMUSEUM_NAME="$1"
            ;;
        -cmi | --chartmuseumImage)
            shift
            CHARTMUSEUM_IMAGE="$1"
            ;;
        -cmit | --chartmuseumImageTag)
            shift
            CHARTMUSEUM_IMAGE_TAG="$1"
            ;;
        -h | --help)
            print_helm_repository_init_usage
            exit 1
            ;;
        *)
            print_helm_repository_init_usage
...
#
# Validates arguments for 'helm repository init' action
#
validate_helm_repository_init_arguments() {
    if [ -z "${CHART_DIR}" ]; then
        echo "[ERROR] Helm chart directory not specified"
        exit 1
    fi
    ...
    if [ -z "${HELM_PASSWORD}" ]; then
        HELM_PASSWORD=$(openssl rand -hex 16)
        HELM_PASSWORD_GENERATED=true
    fi

    if [ -z "${CHARTMUSEUM_NAME}" ]; then
        echo "[ERROR] chartmuseum helm chart not specified"
        exit 1
    fi

    if [ ! -z "${CHARTMUSEUM_NAME}" ] && [[ ( ${CHARTMUSEUM_NAME} != *.tgz ) ]]; then
        echo "[ERROR] Invalid chartmuseum name: ${CHARTMUSEUM_NAME}"
        exit 1
    elif [ ! -z "${CHARTMUSEUM_NAME}" ] && [ ! -e "${CHART_DIR}/${CHARTMUSEUM_NAME}" ]; then
        echo "[ERROR] File does not exist: ${CHART_DIR}/${CHARTMUSEUM_NAME}"
        exit 1
    fi
}

#
# Prints usage menu for 'helm repository init' action
#
print_helm_repository_init_usage() {
    script_name=`basename ${0}`
    echo "Usage: ${script_name} helm repository init ..."
    echo ""
    echo "Configure a private helm chart repository"
    echo ""
    echo "Options:"
    echo "   -u, --username string                    Default helm chart repo user account. Username 'admin' created if not specified"
    echo "   -p, --password string                    Default helm chart repo password. Password auto-generated if not specified"
    echo "   -d, --dir string                         Local directory for helm charts. Default is /tmp/cases/charts"
    echo "   -cm, --chartmuseum string                Name of chartmuseum helm chart. Name should be chartmuseum-{version}.tgz"
    echo "   -cmi, --chartmuseumImage string          Chartmuseum image. (default: ghcr.io/helm/chartmuseum)"
    echo "   -cmit, --chartmuseumImageTag string      Chartmuseum image tag. (default: v0.13.1)"
    echo "   -h, --help                               Print usage information"
    echo ""
}

#
# Validates required tools for 'helm repository init'
#
validate_helm_repository_init_required_tools() {
    # validate required tools - helm
    helm_command=$(command -v helm 2> /dev/null)
    if [ -z "${helm_command}" ];
    then
        echo "[ERROR] Helm (v3) not found. For RHEL, use 'wget https://get.helm.sh/helm-{VERSION}-linux-amd64.tar.gz' to install"
        exit 1
    fi
}
...
do_helm_repository_load() {
    # parses arguments
    parse_helm_repository_load_arguments "$@"

    # validates arguments
    validate_helm_repository_load_arguments

    # validates required tools
    validate_helm_repository_load_required_tools

    # apply oc command
    echo "[INFO] Adding helm repository"
    helm_cmd="helm repo add ${PRIVATE_HELM_REPO} ${HELM_URL}"
    echo "${helm_cmd}"
    eval ${helm_cmd}
    ...
    # apply oc command
    echo "[INFO] Loading helm repo with contents of ${CHART_DIR}"
    for tgz in ${CHART_DIR}/* ; do
      if [[ "${tgz##*\.}" == "tgz" ]]; then
        helm_cmd="helm push ${tgz} ${PRIVATE_HELM_REPO} --force"
        echo "${helm_cmd}"
        eval ${helm_cmd}
      fi
    done
    ...
    echo "${helm_cmd}"
    eval ${helm_cmd}
    echo "[INFO] Charts loaded are: "
    helm_cmd="helm search repo ${PRIVATE_HELM_REPO}"
    echo "${helm_cmd}"
    eval ${helm_cmd}
...
#
# Parses arguments for 'helm repository load' action
#
parse_helm_repository_load_arguments() {
    # process options
    while [[ "$1" != "" ]]; do
        ...
        --url)
            ...
            ;;
        -h | --help)
            print_helm_repository_load_usage
            exit 1
            ;;
        *)
            print_helm_repository_load_usage
...
#
# Prints usage menu for 'helm repository load' action
#
print_helm_repository_load_usage() {
    script_name=`basename ${0}`
    echo "Usage: ${script_name} helm repository load ..."
    echo ""
    echo "Load a local helm chart repository with bundled charts"
    echo ""
    echo "Options:"
    echo "   -u, --username string        Helm chart repo user account."
    echo "   -p, --password string        Helm chart repo password."
    echo "   --url string                 Helm chart repo URL"
    echo "   -d, --dir string             Local directory for helm charts. Default is /tmp/cases/charts"
    echo "   -h, --help                   Print usage information"
    echo ""
}
...
#
# Validates arguments for 'helm repository load' action
#
validate_helm_repository_load_arguments() {
    if [ -z "${CHART_DIR}" ]; then
        echo "[ERROR] Helm chart directory not specified"
    ...
    if [ -z "$(ls -A ${CHART_DIR})" ]; then
        echo "[ERROR] Helm chart directory is empty"
        exit 1
    fi

    if [ -z "${HELM_USERNAME}" ]; then
        echo "[ERROR] Helm chart repo username not specified"
        exit 1
    fi

    if [ -z "${HELM_PASSWORD}" ]; then
        echo "[ERROR] Helm chart repo password not specified"
        exit 1
    fi

    if [ -z "${HELM_URL}" ]; then
        echo "[ERROR] Helm chart repo URL not specified"
        exit 1
    fi
...
#
# Validates required tools for 'helm repository load'
#
validate_helm_repository_load_required_tools() {
    # validate required tools - helm
    helm_command=$(command -v helm 2> /dev/null)
    ...
    # validate required tools - helm
    helm_push_command=$(command -v helm push 2> /dev/null)
    if [ -z "${helm_push_command}" ];
    then
        echo "[ERROR] Helm push not found. For install steps go to https://github.com/chartmuseum/helm-push"
        exit 1
    fi
}
...
do_helm_repository_delete() {
    # parses arguments
    parse_helm_repository_delete_arguments "$@"

    # apply oc command
    echo "[INFO] Deleting private-helm-repo namespace"
    oc_cmd="oc delete project ${PRIVATE_HELM_REPO}"
    echo "${oc_cmd}"
    eval ${oc_cmd}
    ...
    # apply oc command
    echo "[INFO] Removing SCC to private-helm-repo"
    oc_cmd="oc adm policy remove-scc-from-user privileged system:serviceaccount:${PRIVATE_HELM_REPO}:${PRIVATE_HELM_REPO}"
    echo "${oc_cmd}"
    eval ${oc_cmd}
...
#
# Parses arguments for 'helm repository delete' action
#
parse_helm_repository_delete_arguments() {
    # process options
    while [[ "$1" != "" ]]; do
        case "$1" in
        -h | --help)
            print_helm_repository_delete_usage
            exit 1
            ;;
        *)
            print_helm_repository_delete_usage
...
#
# Prints usage menu for 'helm repository delete' action
#
print_helm_repository_delete_usage() {
    script_name=`basename ${0}`
    echo "Usage: ${script_name} helm repository delete ..."
    echo ""
    echo "Delete a private helm chart repository"
    echo ""
    echo "Options:"
    echo "   -h, --help              Print usage information"
    echo ""
...
#
# Validates required tools for 'helm repository delete'
#
validate_helm_repository_delete_required_tools() {
    # validate required tools - helm
    helm_command=$(command -v helm 2> /dev/null)
    if [ -z "${helm_command}" ];
    then
